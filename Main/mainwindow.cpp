...
void MainWindow::countProtocol()
{
    ...
    QString cN = QString::number(kNeuron+1);
    QString oN = QString::number(orderNeuron+1);
...
void MainWindow::intitializeSenses(int numSenses)
{
    ...
    sizeOrderNet.numNeuronCount = SIZE_MAXCOUNTNEURONS;
    ...
    orderNetwork->countNet = new CountNetwork [sizeOrderNet.numNeuronCount];

    for(register int i=0; i<sizeNet.numNeuron; i++) {
        orderNetwork->countNet[i].vectorNetworkCount = new unsigned char [sizeOrderNet.numNeuronCount * 32];
        orderNetwork->countNet[i].vectorPointerCount = new unsigned char [sizeOrderNet.numNeuronCount * 9];
    ...
    try {
        boot(neuralSenses,sizeNet,statistics, orderNetwork);
    } catch (string text) {
        QMessageBox::critical(this,"CUDA ERROR",QString(text.c_str()).append("\n Se cerrara Aplicación"));
        exit(EXIT_FAILURE);
    }
...
int MainWindow::calculateNumNeuron()
{
    ...
    printf("sizeVectorCharacteristic= %d sizeVectorFlags=%d sizeNeuron= %d numNeuron=%d \n",sizeVectorCharacteristic,sizeVectorFlags,sizeNeuron,numNeuron);

    printf("\ndeviceProp.sharedMemperBlock= %d  deviceProp.multiProcessorCount= %d\n", deviceProp.sharedMemPerBlock, deviceProp.multiProcessorCount);
